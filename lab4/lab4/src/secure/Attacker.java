...
public class Attacker{
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("Start typing messages and press Enter!  |  'quit' to exit chat:\n");
        // RECEIVER THREAD 
    ...
    static void sendMessage(String message, DataOutputStream out) throws Exception {
        ...
        String plaintexttimed = plaintext + "||" + timestamp;

        // Step 1 — E(Ks, [IDA, M || T1]): AES-CBC encrypt with group key Ks
        byte[] iv = new byte[16];
        new SecureRandom().nextBytes(iv);
        Cipher aesCipher = Cipher.getInstance("AES/CBC/PKCS5Padding");
        aesCipher.init(Cipher.ENCRYPT_MODE, groupKeyKs, new IvParameterSpec(iv));
        byte[] ciphertext = aesCipher.doFinal(plaintexttimed.getBytes());
    ...
    static String receiveMessage(String senderID, String encPayload, String sigB64) {
        try {
            // ── Decrypt E(Ks, [IDA, M || T_Client]) with group key Ks ────────────────────
            ...
            String decryptMsg = new String(aesCipher.doFinal(ciphertext));

            // Parse: IDA || M || T_Client
            int lastSep = decryptMsg.lastIndexOf("||");
            String plaintext = decryptMsg.substring(0, lastSep);   
            String[] parts  = decryptMsg.split("\\|\\|", 3);
            String timestampStr = "";
            String message = "";
            if(parts.length == 3){
                message = parts[1];
            ...
            // Convert timestamp back to date time object
            ...
            if(timestamp.equals(now)){
                ...
                System.out.println("│  Plaintext      : " + decryptMsg);
                System.out.println("│  SigA(IDA,M)    : " + sigStatus);
            }
            else{
                System.out.println("Replay attack detected! Message not displayed.");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
