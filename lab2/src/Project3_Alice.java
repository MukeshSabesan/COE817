...
public class Project3_Alice {
    ...
    public static void main(String args[]) throws Exception{
        ...
        signature.initSign(alicePrivateKey); //Alice initializes the signature with her Private Key
        signature.update(message.getBytes(StandardCharsets.UTF_8)); // adds the message to the signature
        byte[] signedMessage = signature.sign(); //the full signature is created as Bytes
        String msg1 = message + "||" + Base64.getEncoder().encodeToString(signedMessage) + "||" + nonceA; // message = message + signed(message)
        out.writeUTF(msg1); // the message is passed to Bob.
